...
import { Ban, UserX, ChevronLeft, ChevronRight, TrendingDown, ArrowUpRight, Hash } from 'lucide-react'
import { Skeleton, StatusContratoBadge } from '@/components/ui'
...
export default function CanceladosPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['cancelados', page],
    queryFn: () => clienteApi.cancelados(page, pageSize),
  })
    ...
    <div style={{ paddingBottom: 40 }}>
      ...
      <div className="animate-fade-up" style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
          ...
          <h1 className="text-title" style={{ fontSize: 22 }}>Contratos Cancelados</h1>
        </div>
        <p style={{ fontSize: 14, color: 'var(--text-muted)', marginLeft: 52 }}>
          Clientes com contratos no status cancelado.
        </p>
      </div>
      ...
      <div className="animate-fade-up animate-delay-100">
        {isLoading ? (
          ...
          <div className="card" style={{ padding: 24, borderLeft: '4px solid #a855f7' }}>
            <p style={{ fontSize: 14, fontWeight: 600, color: '#a855f7', marginBottom: 4 }}>Erro de Conexão</p>
          ...
          <div className="card" style={{ padding: '60px 20px', textAlign: 'center' }}>
            ...
            <p className="text-title" style={{ fontSize: 16, marginBottom: 4 }}>Nenhum contrato cancelado</p>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12, marginBottom: 20 }}>
              <MiniStat label="Total Cancelados" value={data.totalElements} color="#a855f7" icon={<TrendingDown size={16} />} />
              <MiniStat label="Nesta Página" value={data.content.length} color="var(--text-primary)" icon={<Hash size={16} />} />
              <MiniStat label="Páginas" value={data.totalPages} color="var(--accent)" icon={<Ban size={16} />} />
            </div>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {data.content.map((cliente) => {
                      const contratoCancelado = cliente.contratos?.find(c => c.statusContrato === 'CANCELADO')
                      return (
                        <tr key={cliente.id}>
                          <td>
                          ...
                          <td>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                              {contratoCancelado ? (
                                <>
                                  <StatusContratoBadge status="CANCELADO" />
                                  {contratoCancelado.tipoContrato && (
                                    <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{contratoCancelado.tipoContrato}</span>
                                  )}
                                </>
                              ) : (
