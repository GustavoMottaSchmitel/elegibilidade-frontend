...
import { ShieldX, UserX, ChevronLeft, ChevronRight, TrendingDown, ArrowUpRight, CalendarClock } from 'lucide-react'
    ...
    <div className="max-w-5xl mx-auto pb-10">
      <div className="mb-8 animate-fade-up">
        <div className="flex items-center gap-3 mb-1">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-status-red-bg border border-status-red/20 shadow-lg shadow-status-red/5">
            <ShieldX size={20} className="text-status-red" />
          </div>
          <h1 className="text-2xl text-premium-title tracking-tight">Clientes Bloqueados</h1>
        </div>
        <p className="text-[var(--dash-text-secondary)] text-sm ml-14">Clientes com atendimento bloqueado (atrasos superiores a 90 dias)</p>
      </div>
      ...
      <div className="animate-fade-up animate-delay-100">
        {isLoading ? (
          ...
          <div className="premium-card bg-status-red-bg border-status-red/20 px-6 py-5">
            ...
            <p className="text-sm text-[var(--dash-text-secondary)]">Não foi possível carregar a lista de bloqueados.</p>
          </div>
        ) : !data?.content.length ? (
          <div className="premium-card px-8 py-20 text-center opacity-60">
            ...
            <p className="text-premium-title text-lg mb-1">Nenhum cliente bloqueado</p>
            <p className="text-sm text-[var(--dash-text-secondary)]">Nenhum registro encontrado no período.</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <MetricCard label="Total Histórico" value={String(data.totalElements)} icon={<TrendingDown size={18} />} color="red" />
              <MetricCard label="Registros Página"  value={String(data.content.length)} icon={<ShieldX size={18} />} color="red" />
                  ...
                  <thead>
                    <tr className="bg-[var(--dash-bg)] border-b border-[var(--dash-border)]">
                      {['Cliente', 'Documento', 'Financeiro', 'Ação / Perfil'].map(h => (
                        <th key={h} className="px-6 py-4 text-premium-muted">{h}</th>
                      ))}
                    </tr>
                  </thead>
                            ...
                            {fin ? (
                              <div className="space-y-1.5">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-[13px] font-mono font-bold text-status-red">{formatMoeda(fin.valorEmAberto)}</span>
                                </div>
                                {fin.diasAtraso > 0 && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className={clsx(
                                      ...
                                      fin.diasAtraso > 90 ? 'text-status-red bg-status-red-bg border-status-red/20'
...
function MetricCard({ label, value, icon, color }: { label: string; value: string; icon: React.ReactNode; color: 'red' | 'blue' }) {
  const isRed = color === 'red'
  return (
    <div className="premium-card flex items-center gap-4 py-4 px-5">
      <div className={clsx(
        "w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border",
        isRed ? "bg-status-red-bg text-status-red border-status-red/10" : "bg-blue-500/10 text-blue-500 border-blue-500/10"
      )}>
        {icon}
      </div>
      <div>
        <p className="text-premium-muted text-[8px]">{label}</p>
        <p className={clsx("text-2xl font-bold font-mono tracking-tight", isRed ? "text-status-red" : "text-[var(--dash-text-primary)]")}>
          {value}
        </p>
      </div>
    </div>
